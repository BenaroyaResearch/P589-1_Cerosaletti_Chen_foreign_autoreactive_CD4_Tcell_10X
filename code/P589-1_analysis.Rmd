...
combinePairs10X <- function(tcrDf) {
  ...
  # Here "sample" is just a placeholder name for the cell’s "barcode"
  pairs <- data.frame(
    CDR3b         = rep("", nPairs),
    CDR3b_nt      = rep("", nPairs),
    TRBV          = rep("", nPairs),
...
plotGenesOfInterestStimFacet <- function(seuratObject, gene, fileName, nCols = 2) {
  pdf(
    file = file.path(plotDir, paste0(fileName, filenameSuffix, ".pdf")),
    width = 6, height = 5
  )

  # Extract the data for the gene of interest
  data <- FetchData(seuratObject, vars = c("umap_1", "umap_2", "stimulationFigures", gene))

  # Create a custom ggplot
  genesOfInterestPlot <- ggplot(data, aes(x = .data$umap_1, y = .data$umap_2, color = .data[[gene]])) +
    geom_point(size = 0.5) +
    scale_color_viridis_c() +
    labs(x = "UMAP 1", y = "UMAP 2") +
    theme(aspect.ratio = 1) +
...
# and each donor’s percentage contribution (relative to the stimulation total) is shown.
...
savePlot(
  plot = dotPlot.tmp,
  plotDir = plotDir,
  filename = paste0(filenameSuffix, nClustString, "_top20ClusterDefiningGenesDotPlotTregs"),
  height = 4,
  width = 7,
  units = "in",
  dpi = 600,
...
### PART 2. Compute per‑cell module scores and plot boxplot+violin/dots
...
# Make a boxplot with jitter (violin‐style dots) for each P390 gene set (faceted) with x-axis = your clusters.
...
write.csv(topMarkers, file.path(dataOutputDir, paste0("top", as.character(nGenes), "_genes_per_cluster_downsampled_Tconv.csv")), quote = FALSE, row.names = TRUE)
```
