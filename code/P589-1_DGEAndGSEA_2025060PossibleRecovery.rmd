...
create_combined_treg_gsea <- function(comparisons) {
  ...
  treg_hallmark_gsea <- GSEA(treg_geneList,
    ...
    minGSSize = 10,
    maxGSSize = 1000,
    pvalueCutoff = 0.05)  # Get ALL pathways with proper FDR correction
...
# Export GSEA results to Excel files
# Create a function to extract and format GSEA results
extract_gsea_results <- function(gsea_obj, term_type) {
  if (nrow(gsea_obj@result) == 0) {
    return(data.frame())
  }
  ...
  results_df <- gsea_obj@result %>%
    ...
    dplyr::rename(term_name = ID) %>%
    dplyr::arrange(p.adjust)

  return(results_df)
}

# Set up comparisons for both cell types
comparisons_export <- list()
comparisons_export[["results_Treg_IARVMicrobial"]] <- results_Treg_IARVMicrobial
comparisons_export[["results_Tconv_IARVMicrobial"]] <- results_Tconv_IARVMicrobial
...
for (comp in names(comparisons_export)) {
  cat("Processing GSEA results for:", comp, "\n")

  # Get gene list
  df <- comparisons_export[[comp]]
  df_sig <- df %>% filter(adj.P.Val <= 0.1)

  if (nrow(df_sig) < 10) {
    cat("Not enough significant genes for", comp, ", skipping.\n")
    next
  }
  ...
  # Extract results
  hallmark_results <- extract_gsea_results(hallmark_gsea, "Hallmark")
  go_results <- extract_gsea_results(go_gsea, "GO")

  # Clean up term names
  if (nrow(hallmark_results) > 0) {
    hallmark_results$term_name <- gsub("HALLMARK_", "", hallmark_results$term_name)
  }

  if (nrow(go_results) > 0) {
    go_results$term_name <- gsub("GOCC_|GOBP_|GOMF_", "", go_results$term_name)
  }

  # Create cell type name for file naming
  cell_type <- ifelse(grepl("Treg", comp), "Treg", "Tconv")

  # Export to Excel files
  if (nrow(hallmark_results) > 0) {
    hallmark_filename <- file.path(dataOutputDir, paste0("GSEA_Hallmark_", cell_type, "_IARvMicrobial_significant.xlsx"))
    openxlsx::write.xlsx(hallmark_results, hallmark_filename, rowNames = FALSE)
    cat("Exported", nrow(hallmark_results), "significant Hallmark terms for", cell_type, "to", hallmark_filename, "\n")
  } else {
    cat("No significant Hallmark terms for", cell_type, "\n")
  }

  if (nrow(go_results) > 0) {
    go_filename <- file.path(dataOutputDir, paste0("GSEA_GO_", cell_type, "_IARvMicrobial_significant.xlsx"))
    openxlsx::write.xlsx(go_results, go_filename, rowNames = FALSE)
    cat("Exported", nrow(go_results), "significant GO terms for", cell_type, "to", go_filename, "\n")
  } else {
    cat("No significant GO terms for", cell_type, "\n")
  }
}
```
...
create_curated_stacked_treg_gsea <- function(comparisons) {
  ...
  hallmark_curated <- treg_hallmark_gsea@result %>%
    ...
    dplyr::select(ID, Description_clean, NES, p.adjust, GeneSet)
